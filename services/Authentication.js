                ...
                if(data.length>0){
                    ...
                    new dataBase().insert(authOptions, function(err, dataResult){
                        if(!err){
                            ...
                            new responseHandler().sendResponse(req, res, "success", response, 200);
                            new sendEmail({subject:"Little Einsteins Inventory : Successful registration",receivers: createAuth.FranchiseId,emailTemplate:"Registration_PasswordReset", sender:"admin@invetory.littleeinsteins.co.in",fromName:"Little Einsteins Inventory",username:createAuth.FranchiseId,password:body.password}).send(createAuth.FranchiseName)
                        }
                    })
                }else{
